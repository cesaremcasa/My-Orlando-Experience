#!/usr/bin/env python3
"""Print the GCP approval gate. Does not authenticate, enable APIs, or deploy."""
...
def render() -> str:
    ...
    lines = [
        "My Orlando AgentOps v0.4.0 GCP preflight (dry-run only)",
        ...
        "",
        "Intended Cloud Run command (not executed):",
        f"gcloud run deploy orlando-agentops --image=<image> --region={location} "
        f"--project={project} --no-allow-unauthenticated --dry-run",
        "",
